...
alias hereitems='(rm -f ~/D/T*/*;cp `ls -d * | grep "^[abfglrjks][^k]*[isarneomb][tdrepylr]$" | tr "\n" " "` ~/D/T* &>~/X; cd ~/D;ls T*/* 2>~/X | u2d | tr "\r" "@" | grep -ion -e "There is a" -e "/.*" | grep -io -e "There is a" -e ":/a.*@$" -e "/[^a].*@$" | tr -d \\n | tr @ \\n | tr ":/" "n " | u2d | tr "\r" " " | u2d | tr "\r" "h" | u2d | tr "\r" "e" | u2d | tr "\r" "r" | u2d | tr "\r" "e" | u2d | tr "\r" ".")'
alias get='(cat >../obj;cd ->~/X;(mv *[cyolhijkauv][sanloimpeb][ryutped] [bclns][oaih][arcot][rdevr]* ~/D/get &>~/X; ls `cat ~/D/obj` &>~/X && echo You cannot take that. || (cd ~/D/get; grep 'all' ~/D/obj &>~/X && (ls * &>~/X && ls [abfgjklrs]*[^s] | xargs echo 'Taken:' | fold -w75 -s || echo Nothing to take.) || (ls ~/D/get/`cat ~/D/obj` &>~/X && echo Taken. || echo I do not see that here.))); (cd ~/D; grep '^[an].*[cd]$' obj &>~/X && echo '[an]*[cd]' > obj; grep '.*e[lm]e.*[td]$' obj &>~/X && echo '*e[lm]e*[td]' > obj; grep '^[bc][ohap].*[dpu]$' obj &>~/X && echo '[bc][ohap]*[dpu]' > obj; grep 'all' obj &>~/X && echo "*" > obj; cd get; mv `cat ../obj` ../inv)&>~/X;mv ~/D/get/* .)2>~/X <'
